...
void getExpType(Type expType, struct TreeNode* currentNode)
{
	...
	if( strcmp( currentNode->token, "Exp" ) == 0 ){
		...
		/*
		 * fix the bug about the operation on the EXP DOT ID
		 */
		if( currentNode->parent->child[currentNode->index + 1] != NULL
			...
			if(firstType->u.structure != NULL){
				structType temp_struct = getstructType(firstType->u.structure->name);		  
				FieldList head = temp_struct->tail;
				int test = 0;
				while(head!=NULL){
					if(strncmp(head->name, currentNode->parent->child[2]->ID, 32)==0) {			  
						test = 1;
						break;
					}			  		  
					head = head->tail;
				...
				if(test == 0){
					printf("Error type 14 : Unexisted field \"%s\"\n", currentNode->parent->child[2]->ID);
					return;
				}
				/*
				 * 2013/05/26
				 * we only deal with the special situation
				 * when the struct only recursive once
				 * assign the type directly...
				 */
				expType->kind = head->type->kind;
				if(expType->kind == basic )
					expType->u.basic = head->type->u.basic;
				else
					expType->u.structure = head->type->u.structure;  
				return;
			} 
		}
		if( currentNode->parent->child[currentNode->index + 1] != NULL
 ...
 * 2013/05/28
 * fix a bug
 * move the deal about the EXP DOT
 * only deal with the opposite situation
 */
