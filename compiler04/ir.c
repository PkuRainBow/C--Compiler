	...
	while (NULL != icn) {
		...
		switch (icp->kind) {
			...
			case IR_PARAM:
				...
				//sprintf(icp->u.uniop.op->u.param_name, "v%d", tmp);
				fprintf(fout,"PARAM %s\n", icp->u.uniop.op->u.param_name);
				break;
			case _ARG:
...
OperandPtr new_param(char *param_name) 
{
	...
	int tmp = getName(param_name)->var_no;
	sprintf(name, "v%d", tmp);
//	printf("param_name %s: intername: %s", param_name, name);
    param->u.param_name = name;
    return param;
}
...
OperandPtr check_param(ICN_Ptr head, int id) {
    ...
    while (NULL != tmp) {
        ...
        if (ic->kind == IR_PARAM) {
			char name[20];
			sprintf(name, "v%d", id);
            if (strcmp((ic->u.uniop.op)->u.param_name, name) == 0) {
